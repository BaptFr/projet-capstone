...
export const fetchAPI = async (date) => {
    const bookingData = localStorage.getItem("bookingByDate");
    const bookingByDate = bookingData ? JSON.parse(bookingData) : [];

    const booked = bookingByDate.filter((b) => b.date === date);
    if (booked.length !== 0) {
    ...
    if (isDateExist) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify([
          ...
          {
            ...newBooking[0],
            booked: [...newBooking[0].booked, time],
          },
        ])
      );
      return true;
